...
{
    private static NetworkPoolManager _instance;

    public static NetworkPoolManager Singleton { get { return _instance; } }
    ...
    public void OnValidate()
    {
        for (var i = 0; i < PooledPrefabsList.Count; i++)
        {
            ...
            if (prefab != null)
            {
                Assert.IsNotNull(prefab.GetComponent<NetworkObject>(), $"{nameof(NetworkPoolManager)}: Pooled prefab \"{prefab.name}\" at index {i.ToString()} has no {nameof(NetworkObject)} component.");
            }
        }
    }
...
{
    GameObject m_Prefab;
    NetworkPoolManager _mNetworkPoolManager;

    public PooledPrefabInstanceHandler(GameObject prefab, NetworkPoolManager networkPoolManager)
    {
        m_Prefab = prefab;
        _mNetworkPoolManager = networkPoolManager;
    }

    NetworkObject INetworkPrefabInstanceHandler.Instantiate(ulong ownerClientId, Vector3 position, Quaternion rotation)
    {
        var netObject = _mNetworkPoolManager.GetNetworkObject(m_Prefab, position, rotation);
        return netObject;
    }

    void INetworkPrefabInstanceHandler.Destroy(NetworkObject networkObject)
    {
        _mNetworkPoolManager.ReturnNetworkObject(networkObject, m_Prefab);
    }
}
